...
def process_image():
    ...
    try:
        ...
        channel_type = data.get("channelType", "awgn").lower()
        k_factor = float(data.get("kFactor", 1.0))
        m_factor = float(data.get("mFactor", 1.0))
        ...
        text = f"SNR: {snr:.1f} dB | {channel_type.upper()} | PSNR: {psnr_val:.4f} dB | SSIM: {str(ssim_val)[:6]} | CosSim: {cos_sim:.4f}"
