...
class RayleighChannel(nn.Module):
    ...
    def forward(self, signal):
        ...
        h_real = torch.randn_like(signal) / np.sqrt(2.0)
        h_imag = torch.randn_like(signal) / np.sqrt(2.0)
        h = torch.sqrt(h_real**2 + h_imag**2)
        
        received = h * signal + noise
        equalized = received / (h + 1e-8)
        return equalized, noise_power.item()

class RicianChannel(nn.Module):
    """Rician Fading Channel"""
    def __init__(self, snr_db=10.0, k_factor=1.0):
        super().__init__()
        self.snr_db = snr_db
    ...
    def forward(self, signal):
        ...
        mean = np.sqrt(K / (K + 1))
        std = np.sqrt(1 / (2 * (K + 1)))
        h_real = mean + std * torch.randn_like(signal)
        h_imag = std * torch.randn_like(signal)
        h = torch.sqrt(h_real**2 + h_imag**2)
...
class NakagamiChannel(nn.Module):
    """Nakagami Fading Channel"""
    def __init__(self, snr_db=10.0, m_factor=1.0):
        super().__init__()
        self.snr_db = snr_db
        self.m_factor = max(m_factor, 0.1)
    ...
    def forward(self, signal):
        ...
        m = self.m_factor
        gamma_dist = torch.distributions.gamma.Gamma(m, m)
        h_squared = gamma_dist.rsample(signal.shape).to(signal.device)
        h = torch.sqrt(h_squared)
...
class ChannelFactory:
    @staticmethod
    def get_channel(channel_type, snr_db, k_factor=1.0, m_factor=1.0):
        channel_type = channel_type.lower()
        if channel_type == 'rayleigh':
            return RayleighChannel(snr_db)
        elif channel_type == 'rician':
            return RicianChannel(snr_db, k_factor)
        elif channel_type == 'nakagami':
            return NakagamiChannel(snr_db, m_factor)
        else:
...
class SemanticCommNetwork:
    ...
    def transmit_receive(self, image, snr_db=10.0, channel_type='awgn', k_factor=1.0, m_factor=1.0, add_noise=True, seed=None):
        """
        ...
        """
        channel = ChannelFactory.get_channel(channel_type, snr_db, k_factor, m_factor)
        ...
        with torch.no_grad():
            ...
            # Channel
            if add_noise:
                semantic_noisy, noise_power = channel(semantic)
            else:
